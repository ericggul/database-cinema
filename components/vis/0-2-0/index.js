...
function AtlasCubeGrid({ onHover, onClick }) {
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < 12; y++) {
      for (let z = 0; z < 12; z++) {
        // Layout: Flat grid on Y-Z plane (since X was year)
        // Or X-Y plane?
        // Let's do X=Month, Y=Hour for a 2D wall
        
        const xPos = y - 5.5;
        const yPos = z - 5.5;
        const zPos = 0;
        
        tempObject.position.set(xPos * 1.2, yPos * 1.2, zPos);
    ...
    <Container>
      <div style={{
        ...
        >
          ...
          <gridHelper args={[30, 30, 0x444444, 0x222222]} rotation={[Math.PI/2, 0, 0]} position={[0, 0, -1]} />
        </Canvas>
      </div>

      <Overlay>
        <Title>1995 Bukhansan (144 Images)</Title>
        <Info>
          Grid: 12 (Month) x 12 (Hour)<br/>
          <br/>
          {activeItem ? (
            <>
