...
export default function SceneTransformerControlNet() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoadingBase(true);
        const resp = await fetch("/img/test3.png");
  ...
  const buildPrompt = (year, month, hour) => {
    ...
    return `Realistic view of Bulgwangcheon in Eunpyeong-gu. Preserve exact perspective, bridges, river path, banks, and skyline from the source image. Render the scene precisely at ${month} ${year}, ${hourText}. Enforce era-appropriate appearance for that year, season cues for that month, and lighting/sky consistent with that time of day (night is dark with artificial lights; 22:00 must look like late night; noon must look like strong daylight). Do not change camera angle or composition. ${additionalPrompt ? `Additional directives: ${additionalPrompt}` : ""}`;
  };
  ...
  const handleBatchGenerate = async () => {
    ...
    for (let i = 0; i < plan.length; i += 1) {
      ...
      try {
        ...
        const response = await fetch("/api/replicate/transform-controlnet", {
          method: "POST",
  ...
  return (
    <Container>
      <Title>Scene Time Machine · Replicate (SDXL ControlNet)</Title>
