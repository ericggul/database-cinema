...
export default function SceneTransformerControlNetStrong() {
  ...
  const buildPrompt = (year, month, hour) => {
    ...
    return `Realistic Bulgwangcheon in Eunpyeong-gu. Preserve the exact perspective, bridges, river path, banks, and skyline of the source image. Depict the scene at ${month} ${year}, ${hourText}. Enforce era-accurate details for that year, seasonal cues for that month, and lighting/sky consistent with that time of day (22:00 must be late night with artificial lighting; noon must be bright daylight). Do not alter camera angle or composition. ${additionalPrompt ? `Additional directives: ${additionalPrompt}` : ""}`;
  };
  ...
  const handleBatchGenerate = async () => {
    ...
    for (let i = 0; i < plan.length; i += 1) {
      ...
      try {
        ...
        const response = await fetch("/api/replicate/transform-controlnet-strong", {
          method: "POST",
  ...
  return (
    <Container>
      <Title>Scene Time Machine · ControlNet (strong)</Title>
