...
export default function SceneTransformerStandard() {
  ...
  const buildPrompt = (year, month, hour) => {
    ...
    return `Transform this scene to represent ${month} ${year} at ${hourText}.
Focus on correct lighting, shadows, seasonal cues, and era-accurate atmosphere.
...
Keep composition and structures consistent with the source photo.`;
  ...
  const handleBatchGenerate = async () => {
    ...
    for (let i = 0; i < plan.length; i += 1) {
      ...
      try {
        ...
        const response = await fetch("/api/dalle/transform-3", {
          method: "POST",
  ...
  return (
    <Container>
      <Title>Scene Time Machine · Standard (Fast)</Title>
