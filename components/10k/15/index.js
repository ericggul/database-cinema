...
export default function SceneTransformerRealVisControl() {
  ...
  const buildPrompt = (year, month, hour) => {
    ...
    return `Realistic Bulgwangcheon in Eunpyeong-gu, keep exact perspective/bridge/river layout. Time: ${month} ${year}, ${hourText}. Adjust only lighting/season/time cues. ${additionalPrompt ? `Additional directives: ${additionalPrompt}` : ""}`;
  };
  ...
  const handleBatchGenerate = async () => {
    ...
    for (let i = 0; i < plan.length; i += 1) {
      ...
      try {
        ...
        const response = await fetch("/api/replicate/transform-realvis-control", {
          method: "POST",
  ...
  return (
    <Container>
      <Title>Scene Time Machine · RealVis + Control</Title>
