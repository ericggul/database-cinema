...
export default function SceneTransformer() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoadingBase(true);
        const resp = await fetch("/img/test.jpg");
  ...
  const downscaleDataUrl = (dataUrl, maxSize = 512, quality = 0.85) =>
  ...
  const buildPrompt = (year, month, hour) => {
    ...
    return `Transform this scene to accurately represent ${month} ${year} at ${hourText}. 
Emphasize lighting, shadows, seasonal cues, and architectural/weather context authentic to that period and time.
${additionalPrompt ? `Additional directives: ${additionalPrompt}` : ""}
Keep composition and structures consistent with the source photo.`;
  };
  ...
  const handleBatchGenerate = async () => {
    ...
    setRunning(true);
    const compactBase = await downscaleDataUrl(baseImage, 512, 0.85);
  ...
  return (
    <Container>
      <Title>Scene Time Machine · Batch Axis</Title>
      ...
      <Controls>
        ...
        <ControlGroup style={{ width: "100%" }}>
          ...
          <Input
            ...
            placeholder="e.g. emphasize wet streets, neon signage, rainy mood"
            style={{ width: "100%" }}
          />
        </ControlGroup>
      ...
      <PreviewContainer>
        <ImageBox>
          <ImageLabel>Base Scene</ImageLabel>
          {baseImage ? (
