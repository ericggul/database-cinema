...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!createRes.ok) {
      ...
      console.error("[/api/replicate/transform-controlnet-strong] create failed", text);
    ...
    if (!url) {
      console.error("[/api/replicate/transform-controlnet-strong] no output", result);
      return res.status(500).json({ message: "No image generated" });
    }

    return res.status(200).json({ imageUrl: url });
  } catch (error) {
    console.error("[/api/replicate/transform-controlnet-strong] error", error?.message, error?.stack);
