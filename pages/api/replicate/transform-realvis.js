...
export default async function handler(req, res) {
  ...
  try {
    ...
    const model = "adirik/realvisxl-v3.0-turbo";
    ...
    if (!createRes.ok) {
      ...
      console.error("[/api/replicate/transform-realvis] create failed", text);
    ...
    if (!url) {
      console.error("[/api/replicate/transform-realvis] no output", result);
      return res.status(500).json({ message: "No image generated" });
    }

    return res.status(200).json({ imageUrl: url });
  } catch (error) {
    console.error("[/api/replicate/transform-realvis] error", error?.message, error?.stack);
