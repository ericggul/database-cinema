...
async function pollPrediction(id, headers, maxMs = 120000, intervalMs = 1500) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const model = "sdxl-based/realvisxl-v3-multi-controlnet-lora";
    ...
    if (!createRes.ok) {
      ...
      console.error("[/api/replicate/transform-realvis-control] create failed", text);
      return res.status(createRes.status).json({ message: "Replicate create failed", details: text });
    }

    const created = await createRes.json();
    const predictionId = created.id;
    const result = await pollPrediction(predictionId, headers, 120000, 1500);
    const url = result?.output?.[0];
    if (!url) {
      console.error("[/api/replicate/transform-realvis-control] no output", result);
      return res.status(500).json({ message: "No image generated" });
    }

    return res.status(200).json({ imageUrl: url });
  } catch (error) {
    console.error("[/api/replicate/transform-realvis-control] error", error?.message, error?.stack);
